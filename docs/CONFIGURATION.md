...
### PgCat (`pgcat_sync_topology.py.j2` -> `/etc/pgcat/pgcat.toml`)

The SQL-aware proxy that gives you real per-statement read/write splitting -
one endpoint, no dual-port app logic required. Full explanation in
[PGCAT.md](PGCAT.md). Quick reference:
